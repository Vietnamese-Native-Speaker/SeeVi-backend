...
pub trait CVDataSource {
    ...
    async fn get_cv_by_id(&self, _id: ObjectId) -> Result<CV, DataSourceError>;

    /// Add CV with the provided input.
    async fn create_cv(&self, _input: CreateCVInput) -> Result<CV, DataSourceError>;

    /// Update the CV with the provided input, using the id in the input as
    /// finder.
    async fn update_cv_info(&self, _input: UpdateCVInput) -> Result<CV, DataSourceError>;

    /// Delete the CV with the provided id.
    async fn delete_cv(&self, _id: ObjectId) -> Result<(), DataSourceError>;
}
