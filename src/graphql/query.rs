...
impl Query {
    ...
    > {
        todo!()
        // let db = ctx.data_unchecked::<MongoDB>();
        // let friends_list = Vec::<Result<User, UserDataSourceError>>::new();
        // connection::query(
        //     after,
        //     before,
        //     first,
    ...
    > {
        let db = ctx.data_unchecked::<MongoDB>();
        let cvs = Vec::<Result<CV, CVDataSourceError>>::new();
        connection::query(
            after,
            before,
            first,
            ...
            |after, before, first, last| async move {
                ...
                connection
                    .edges
                    .extend(cvs.into_iter().map(|cv| connection::Edge::new(cv.id, cv)));
                Ok::<_, async_graphql::Error>(connection)
            },
        )
        .await
